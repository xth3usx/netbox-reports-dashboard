...
function getVMsSemSite($url, $headers) {
    $vms_sem_site = [];
    $next = "$url/virtualization/virtual-machines/?limit=100&offset=0&expand=site";
    ...
    while ($next) {
        ...
        foreach ($data['results'] as $vm) {
            if (empty($vm['site'])) {
                $vms_sem_site[] = strtolower(trim($vm['name']));
            }
        }
...
$vms_sem_site = getVMsSemSite($netbox_url, $headers);

if (isset($_GET['export']) && $_GET['export'] === 'xls') {
    header("Content-Type: application/vnd.ms-excel");
    header("Content-Disposition: attachment; filename=vms_sem_site.xls");
    echo "Nome da VM\n";
    foreach ($vms_sem_site as $vm) {
        echo "$vm\n";
    }
    exit;
}
...
<head>
    <meta charset="utf-8">
    <title>NetBox - VMs sem Site</title>
...
<main>
  <section class="vm-list">
    <h2 style="font-size: 1.2rem;">Máquinas virtuais sem <span style="color: #f57c00;">Site</span> atribuído</h2>
    <p style="font-size: 0.85rem; color: #666;">Relatório gerado em: <?= $report_date ?></p>
    <p>Total: <strong><?= count($vms_sem_site) ?></strong></p>
    <ul>
      <?php foreach ($vms_sem_site as $vm): ?>
        <li class="entry">
